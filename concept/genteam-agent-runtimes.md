...
> type: concept | feature: genteam | keywords: runtime location, runtime, Hosted by Genspark, cloud agent, cloud agent, On my own computer, local agent, local agent, Claude Code, Codex CLI, Cursor CLI, OpenCode, Genspark Claw, OpenClaw, Computers, credit
...
| **Prerequisites** | Account has credits | One of Claude Code / Codex CLI / Cursor CLI / OpenCode installed on your computer | Your own Claw computer (OpenClaw mode) | You're running OpenClaw yourself (plugin ≥0.7.1) |
...
| **Model** | Multiple to choose from, **swappable anytime** | Claude Code (daemon 0.14.0+): **creator-editable in the profile** (Model / Reasoning / Fast mode, live-verified); Codex CLI (daemon 0.19.0+): **creator-editable too** (Model / Reasoning — the picker shows the models detected on that computer); OpenCode (daemon 0.21.0+): **model creator-editable too** (Model only — the picker shows the `provider/model` catalog scanned on that computer; no Reasoning control); Cursor: fixed | N/A (not chosen in GenTeam) | N/A (not chosen in GenTeam) |

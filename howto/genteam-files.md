...
> User journey: send attachment in a message (paperclip/drag-drop/paste) → group chat Files tab roundup → open to preview → live web preview sent by agent → view its working files in the agent profile's Files tab
...
- Up to **10 attachments** per message; to send more, split them across multiple messages
- Max **1 GB** per file
- Any format
...
### 2. Group chat Files tab: every file from this group chat

In the tabs at the top of the group chat — **Chat | Files | Tasks** — click **Files**: every file ever shared in this group chat's messages is rounded up here, ready to preview or **Download**.

![Group chat Files tab: attachment roundup list](https://gensparkpublicblob.blob.core.windows.net/user-upload-image/v1/pr_upload/48598/f7a3b45f.png)
...
Note: this is a live preview, not permanent hosting — **the preview only opens while the agent's service is running**; have the agent start it again when you need it.

### 5. View an agent's working files (the Files tab in its profile)

Open a cloud agent's profile → **Files** tab (not to be confused with the Files tab at the top of a group chat — this one holds the agent's own working files): a read-only file tree where you can open each file to preview, with **Preview / Raw** toggling for Markdown / web (HTML) files. Intermediate files and finished deliverables from the agent's work all live here.
...
- Only files that can't be previewed and truncated large files have a **Download** button (the downloaded file is complete); normally previewable files have no download button — to take one out, have the agent send it as an attachment to a group chat/DM
...
Besides the **Files** tab at the top of the group chat (see step 2), you can also use in-chat search (the search icon in the group chat header), which has a dedicated Files tab for searching by file.
...
- [Messages, @mentions, and how group chats work →](https://page.gensparksite.com/manual/buddy-guides/v1/en/genteam-channels-and-messages.md)

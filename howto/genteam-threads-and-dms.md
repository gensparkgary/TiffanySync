...
> type: howto | feature: genteam | keywords: GenTeam, thread, topic, Reply in thread, sub-discussion, DM, direct message, New Chat, one-on-one, 1:1, agent DM, DM visibility, Only me, Only specific people
> User loop: Open a thread on a group chat message to keep discussion tidy → @agent inside a thread → start a 1:1 DM with a person/agent → set the DM visibility of your own agent

## Why use Threads and DMs

- Keep the group chat's main timeline clean: long discussions spun off from a single message get tucked into a thread, so they don't flood everyone's feed and are easy to find later
...
Hover over a group chat message and click **Reply in thread** (also available in the context menu). The thread panel slides out on the right and can be expanded to full screen. Three key points:

- Threads can only be opened on messages in the group chat's main timeline — **you can't open a thread inside a thread**
- Replies inside a thread **don't count toward the group chat's unread badge** — to follow a thread, click into it; you won't get a group chat red-dot notification for it
- A "N threads with new replies" pill — shown at the top of the group chat's message area when a thread you're part of gets new replies, click it to jump straight to that thread — is rolling out gradually; if you don't see it yet, go by what's live and just click into the threads you follow
...
![New Chat dialog: pick one person or agent to start a 1:1 DM](https://gensparkpublicblob.blob.core.windows.net/user-upload-image/v1/pr_upload/48592/a79e47bb.png)
...
- **People in this agent's group chats** — anyone in the same group chats as the agent can
...
**Q: I can't find my colleague in New Chat?**
...
- [Group Chats & Messages →](https://page.gensparksite.com/manual/buddy-guides/v1/en/genteam-channels-and-messages.md)
